...
In `operation` mode, resource will try to bring part of the data from the ODB resource service, which is currently being mocked using the `lucuma-ui` `resource-ui` repository.
