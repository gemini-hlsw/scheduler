...
def compute_visibility(programs_ids: Optional[bytes] = None,
                       semester: Semester = Semester(year='2018', half=SemesterHalf('B')),
    ...
    start = Time(f"{semester.start_date()} 08:00:00", format='iso', scale='utc')
    end = Time(f"{semester.end_date()} 08:00:00", format='iso', scale='utc')
    semesters = frozenset([semester])

    sites = ALL_SITES

    if programs_ids is None:
    ...
    return (vis_table, parsed_observations)
...
async def calculate_and_save():
    sem = Semester(year='2018', half=SemesterHalf('B'))
    slot_length = 1.0
    
    (vis_table, parsed_observations) = compute_visibility(programs_ids=None, semester=sem, slot_length=slot_length)

    main_key = f"{sem}-{slot_length}min"
