...
class PDRQueue:
    ...
    async def put(
        ...
        async with self.lock:
            ...
            task = asyncio.create_task(self._auto_pop(event.resource_name, timer_duration, callback))
...
class ResourceEventHandler(EventHandler):
    ...
    @staticmethod
    async def _disabled_callback(event: MockResourceEvent):
        ...
        match event.resource_status:
            case "disabled":
                ...
                if event.resource_name in last_plan.resources():
                    await self.pdr.put(event, self._disabled_callback)
            case "enabled":
