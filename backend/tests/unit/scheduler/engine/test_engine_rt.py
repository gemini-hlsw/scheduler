...
"""RT-25/RT-26: EngineRT offloads build/compute to an executor; the event loop
must stay responsive, and a hung worker times out and gets recycled."""
...
def make_engine(executor_factory=None, process_id="rt-test"):
    params = MagicMock()
    ...
    return EngineRT(params, MagicMock(), process_id,
                    weather_source=weather_source, executor_factory=executor_factory)
...
async def test_event_loop_stays_responsive_while_worker_computes(rt_event_factory, mock_coordination_idle):
    ...
    with ThreadPoolExecutor(max_workers=1) as pool:
        engine = make_engine(executor_factory=lambda: pool)
...
def test_plan_timeout_default_and_env_override(monkeypatch):
    """operation.plan_timeout_seconds defaults to 60 and follows the
    PLAN_TIMEOUT_SECONDS env var dynamically (resolved on each read)."""
    engine = make_engine()

    monkeypatch.delenv("PLAN_TIMEOUT_SECONDS", raising=False)
    assert engine._plan_timeout() == 60.0

    monkeypatch.setenv("PLAN_TIMEOUT_SECONDS", "12.5")
    assert engine._plan_timeout() == 12.5


@pytest.mark.asyncio
async def test_hung_worker_times_out_and_recycles_pool(rt_event_factory, mock_coordination_idle, monkeypatch):
    """A worker that exceeds the plan budget raises TimeoutError and the pool
    is recycled so the next plan gets a fresh worker."""
    monkeypatch.setenv("PLAN_TIMEOUT_SECONDS", "0.1")
    ...
    def hanging_compute(payload):
        time.sleep(0.5)
        return MagicMock()

    engine = make_engine(executor_factory=factory)

    try:
        with patch('scheduler.engine.engineRT.worker_build', lambda p, b: True), \
             patch('scheduler.engine.engineRT.worker_compute', hanging_compute), \
             patch('scheduler.engine.engineRT.SPlans'):
            with pytest.raises(TimeoutError):
                await engine.compute_event_plan(rt_event_factory.on_demand())

        assert engine._executor is None, "pool must be recycled after a timeout"
        # The interlock's plan-in-progress flag was still cleared.
        mock_coordination_idle.signal_plan_done.assert_awaited_once()
    finally:
        for pool in pools:
...
@pytest.mark.asyncio
async def test_run_publishes_night_plans_error_on_timeout(rt_event_factory, scheduler_queue,
                                                          mock_coordination_idle, monkeypatch):
    """End to end through EngineRT.run: a timed-out plan reaches subscribers
    as a NightPlansError and the engine keeps consuming events."""
    ...
    def compute(payload):
        calls['n'] += 1
        if calls['n'] == 1:
            time.sleep(0.5)  # first plan hangs past the budget
        return MagicMock()

    engine = make_engine(executor_factory=factory, process_id="rt26-run")
    engine.scheduler_queue = scheduler_queue

    subscriber = asyncio.Queue()
    plan_response_subscribers["rt26-run"] = {subscriber}

    await scheduler_queue.add_schedule_event(rt_event_factory.on_demand())
    await scheduler_queue.add_schedule_event(rt_event_factory.end_of_night())

    try:
        with patch('scheduler.engine.engineRT.worker_build', lambda p, b: True), \
             patch('scheduler.engine.engineRT.worker_compute', compute), \
             patch('scheduler.engine.engineRT.SPlans'):
            await asyncio.wait_for(engine.run(), timeout=5)

        published = subscriber.get_nowait()
        assert isinstance(published, NightPlansError)
        assert "exceeded" in published.error
        assert len(pools) == 2, "second plan must run on a fresh pool"
    finally:
        plan_response_subscribers.pop("rt26-run", None)
        engine.shutdown_workers()
        for pool in pools:
            pool.shutdown(wait=False)
