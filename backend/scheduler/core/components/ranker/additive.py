...
__all__ = ["AdditiveRanker", "RankerParameters", "RankerBandParameters"]
...
def _default_score_combiner(x: npt.NDArray[float]) -> npt.NDArray[float]:
    ...
    return np.array([np.max(x)]) if 0 not in x else np.array([0.0])
...
_def_alt_limits: Dict[MinMax, Angle] = {
    MinMax.MIN: Angle(18.0 * u.deg),
...
class RankerParameters:
    ...
    gs_altitude_limits: Dict[MinMax, Angle] = field(
        default_factory=lambda: _def_alt_limits
    )
    gn_altitude_limits: Dict[MinMax, Angle] = field(
        default_factory=lambda: _def_alt_limits
    )
    ...
    dec_diff_less_40: npt.NDArray[float] = field(
        default_factory=lambda: np.array([3.0, 0.0, -0.08])
    )

    # HA weighting for zenith distances > 40 deg (Xmin > 1.3)
    # Weighted to 0, the original intention
    dec_diff: npt.NDArray[float] = field(
        default_factory=lambda: np.array([3.0, 0.0, -0.08])
    )
    ...
    score_combiner: Callable[[npt.NDArray[float]], npt.NDArray[float]] = field(
        init=False
    )
    ...
    def __post_init__(self):
        ...
        self.altitude_limits = {
            Site.GS: self.gs_altitude_limits,
            Site.GN: self.gn_altitude_limits,
        }

        for site in ALL_SITES:
            if self.altitude_limits[site][MinMax.MIN] < Angle(18.0 * u.deg):
                raise ValueError(
                    f"The minimum altitude limit for {site.name} must be at least 18 degrees."
                )
            if self.altitude_limits[site][MinMax.MAX] > Angle(90.0 * u.deg):
                raise ValueError(
                    f"The maximum altitude limit for {site.name} must be 90 degrees or less."
                )
                ...
                for midx, minmax in enumerate(self.altitude_limits[site]):
                    if midx != len(self.altitude_limits[site]) - 1:
                        text += (
                            ...
                            + str(minmax.name)
                            + ": "
                            + str(self.altitude_limits[site][minmax].value)
                            + " deg"
                        )
                    else:
                        text += (
...
def _default_band_params() -> RankerBandParameterMap:
    ...
    params = {
        Band.BAND4: RankerBandParameters(
            m1=0.00, b1=0.1, m2=0.00, b2=0.0, xb=0.8, xb0=0.0, xc0=0.0
        )
    }
    for band in [Band.BAND3, Band.BAND2, Band.BAND1]:
        ...
        params[band] = RankerBandParameters(
            m1=m1, b1=b1, m2=m2[band], b2=b2, xb=xb, xb0=0.0, xc0=0.0
        )
...
class AdditiveRanker(Ranker):
    ...
    def metric_slope(
        ...
        if len(band) != len(completion):
            raise ValueError(
                f"Incompatible lengths (band={len(band)}, completion={len(completion)}) between band "
        ...
        for idx, curr_band in enumerate(band):
            ...
            if self.params.power == 1:
                b2 = (
                    xb
                    * (self.band_params[curr_band].m1 - self.band_params[curr_band].m2)
                    + self.band_params[curr_band].xb0
                    + self.band_params[curr_band].b1
                )
            elif self.params.power == 2:
                b2 = (
                    self.band_params[curr_band].b2
                    + self.band_params[curr_band].xb0
                    + self.band_params[curr_band].b1
                )
            ...
            if completion[idx] <= eps:
                ...
                metric[idx] = (
                    self.band_params[curr_band].m1
                    * completion[idx] ** self.params.power
                    + self.band_params[curr_band].b1
                )
                metric_slope[idx] = (
                    self.params.power
                ...
                metric[idx] = (
                    self.band_params[curr_band].m2 * 1.0
                    + b2
                    + self.band_params[curr_band].xc0
                )
                metric_slope[idx] = self.band_params[curr_band].m2
    ...
    def score_observation(
        ...
        cplt = (program.total_used(obs.band) + remaining) / program.total_awarded(
            obs.band
        )

        metric, metric_s = self.metric_slope(
            np.array([cplt]),
        ...
        airmass = {
            night_idx: target_info[night_idx].airmass for night_idx in obs_nights
        }

        # Get the latitude associated with the site.
        site_latitude = obs.site.location.lat
        if site_latitude < 0.0 * u.deg:
            dec_diff = {
                night_idx: np.abs(site_latitude - np.max(dec[night_idx]))
                for night_idx in obs_nights
            }
        else:
            dec_diff = {
                night_idx: np.abs(np.min(dec[night_idx]) - site_latitude)
                for night_idx in obs_nights
            }

        c = {
            night_idx: self.params.dec_diff_less_40
            if angle < 40.0 * u.deg
            else self.params.dec_diff
        ...
        wha = {
            night_idx: c[night_idx][0]
            + c[night_idx][1] * ha[night_idx] / u.hourangle
            + (c[night_idx][2] / u.hourangle**2) * ha[night_idx] ** 2
            for night_idx in obs_nights
        }
        kk = {night_idx: np.where(wha[night_idx] <= 0.0)[0] for night_idx in obs_nights}
        for night_idx in obs_nights:
            wha[night_idx][kk[night_idx]] = 0.0
        ...
        alt_include = {
            night_idx: np.ones(len(targ_alt[night_idx])) for night_idx in obs_nights
        }
        jj = {
            night_idx: np.where(
                np.logical_or(
                    targ_alt[night_idx]
                    < self.params.altitude_limits[obs.site][MinMax.MIN],
                    targ_alt[night_idx]
                    > self.params.altitude_limits[obs.site][MinMax.MAX],
                )
            )[0]
            for night_idx in obs_nights
        }
        ...
        scale_factor *= (
            1.0
            + (priority_value - program.mean_priority()) / self.params.priority_factor
        )
        ...
        prog_priority = {
            night_idx: self.params.program_priority
            if nc[night_idx].filter.program_priority_filter_any(program)
            else 1.0
            for night_idx in obs_nights
        }
    ...
    def _score_and_group(self, group: Group, group_data_map):
        ...
        if len(group.sites()) != 1:
            raise ValueError(
                f"AND group {group.group_name} has too many sites: {len(group.sites())}"
            )
        ...
        nights_to_schedule = list(
            list(group_data_map.values())[0].group_info.scores.keys()
        )
        for night_idx in nights_to_schedule:
            ...
            for unique_group_id in (g.unique_id for g in group.children):
                ...
                subgroup_scores = np.array(
                    [group_data_map[unique_group_id].group_info.scores[night_idx]]
                )
                scores[night_idx] = np.append(
                    scores[night_idx], subgroup_scores, axis=0
                )
        ...
        combine_scores = {
            night_idx: np.apply_along_axis(
                self.params.score_combiner, 0, scores[night_idx]
            )[0]
            for night_idx in nights_to_schedule
        }
