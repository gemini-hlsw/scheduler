...
class OpsEnvService(ExternalService):
    """
    This is a mock Environment service used for GPP operations purposes.
    """
    ...
    def __init__(self, sites: FrozenSet[Site] = ALL_SITES):
        ...
        path = Path(ROOT_DIR) / 'scheduler' / 'services' / 'environment' / 'data' / 'operation'
        for site in self._sites:
            ...
            df[OpsEnvService._night_time_stamp_col] = pd.to_datetime(df[OpsEnvService._night_time_stamp_col])
            df[OpsEnvService._local_time_stamp_col] = pd.to_datetime(df[OpsEnvService._local_time_stamp_col])
            ...
            with open(initial_conditions_path, 'r') as conditions_file:
                ...
                df[OpsEnvService._night_time_col_initial] = pd.to_datetime(df[OpsEnvService._night_time_col_initial],
    ...
    def _convert_to_variant(row) -> (datetime, VariantSnapshot):
        ...
        """
        timestamp = row[OpsEnvService._local_time_stamp_col].to_pydatetime()
        iq = ImageQuality(row[OpsEnvService._iq_col])
        cc = CloudCover(row[OpsEnvService._cc_col])
        wind_dir = Angle(row[OpsEnvService._wind_dir_col], unit=u.deg)
        wind_spd = row[OpsEnvService._wind_speed_col] * (u.m / u.s)
    ...
    def get_variant_changes_for_night(self,
        ...
        filtered_df = df[df[OpsEnvService._night_time_stamp_col].dt.date == night_date]
        variant_list = filtered_df.apply(OpsEnvService._convert_to_variant, axis=1).values.tolist()
    ...
    def get_initial_conditions(self,
        ...
        filtered_df = df[df[OpsEnvService._night_time_col_initial].dt.date == ut_night_date]
