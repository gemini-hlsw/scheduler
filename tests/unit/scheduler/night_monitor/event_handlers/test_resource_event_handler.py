...
class TestPDRQueue:
    ...
    @pytest.mark.asyncio
    async def test_put_adds_event_to_queue(self, queue, sample_event):
        callback = AsyncMock()
        await queue.put(sample_event, callback)
    ...
    @pytest.mark.asyncio
    async def test_put_creates_auto_pop_task(self, queue, sample_event):
        callback = AsyncMock()
        await queue.put(sample_event, callback)
    ...
    @pytest.mark.asyncio
    async def test_auto_pop_triggers_after_timeout(self, sample_event):
        pdr = PDRQueue(timeout=0.1)
        callback = AsyncMock()

        await pdr.put(sample_event, callback, timeout=0.1)
    ...
    @pytest.mark.asyncio
    async def test_force_pop_removes_event(self, queue, sample_event):
        callback = AsyncMock()
        await queue.put(sample_event, callback)
    ...
    @pytest.mark.asyncio
    async def test_force_pop_cancels_auto_pop_task(self, queue, sample_event):
        callback = AsyncMock()
        await queue.put(sample_event, callback, timeout=0.5)
    ...
    @pytest.mark.asyncio
    async def test_clear_removes_all_items(self, queue):
        callback = AsyncMock()
        ...
        await queue.put(event1, callback)
        await queue.put(event2, callback)
...
class TestResourceEventHandler:
    ...
    @pytest.mark.asyncio
    async def test_on_resource_edit_enabled_removes_from_pdr(self, handler, mock_last_plan):
    ...
    @pytest.mark.asyncio
    async def test_on_resource_edit_unknown_status_raises(self, handler):
    ...
    @pytest.mark.asyncio
    async def test_enabled_before_timeout_prevents_callback(self, handler):
    ...
    @pytest.mark.asyncio
    async def test_timeout_triggers_callback(self, handler):
        handler.pdr = PDRQueue(timeout=0.1)
        callback_called = asyncio.Event()
        received_event = None

        async def test_callback(event: MockResourceEvent):
            nonlocal received_event
            received_event = event
            callback_called.set()

        disabled = MockResourceEvent(resource_status="disabled", resource_name="test_resource")
        await handler.pdr.put(disabled, test_callback, timeout=0.1)
