...
from common.queries import Session, test_subscription_query, connection
...
class App:
    ...
    async def run(self):
        ...
        while not done.is_set():
            ...
            # Check for updates in the ODB
            resp = await self.session.subscribe_all(self.config.graphql.url)

            if resp:
                print(f"Change on ODB: {resp['observationEdit']['editType']} for {resp['observationEdit']['value']['id']} ")
